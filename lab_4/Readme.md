...
Фигуры являются фигурами вращения (равнобедренными). Для хранения координат фигур необходимо реализовать свой шаблон template <class T> Point,в качестве параметра шаблона должен быть тип для переменных координат. Проверку на то, что тип T скалярный реализовать с помощью concept. Разработать классы согласно варианту задания, классы должны наследоваться от базового шаблонного класса Figure<T>, где T скалярный тип подставляющийся в Point. Сами точки должны хранится в виде умных указателей std::unique_ptr<Point<T>>. Все классы должны поддерживать набор общих методов.
...
• Вызывать для всего массива общие функции. Т.е. распечатывать для каждой фигуры в
...
В7: 6-угольник, 8-угольник, Треугольник